package com.ywvision.wyvisionhelper.widget.tagview;
...
import static com.ywvision.wyvisionhelper.widget.tagview.Utils.dp2px;
import static com.ywvision.wyvisionhelper.widget.tagview.Utils.sp2px;


public class TagContainerLayout extends ViewGroup implements Filterable {
...
//        tagView.setTagTextColor(colors[2]);
    ...
    private void onRemoveTag(int position) {
        ...
        mTags.remove(mTags.get(position));
        if (!ValidUtil.isEmpty(originalList)) {
            if (position > originalList.size()) {
                originalList.remove(originalList.get(position));
            }
        }
        for (int i = position; i < mChildViews.size(); i++) {
            mChildViews.get(i).setTag(i);
    ...
    private Filter searchFilter = new Filter() {
        ...
        protected FilterResults performFiltering(CharSequence charSequence) {
            ...
            if (charSequence.length() == 0) {
                mTags.addAll(originalList);
            } else {
                String filterPattern = charSequence.toString().toLowerCase().trim();
                for (String searchItem : originalList) {
                    if (searchItem.toLowerCase().trim().contains(filterPattern)) {
                        mTags.add(searchItem);
                    }
                }
            }
            results.values = new ArrayList<>(mTags);
        ...
        protected void publishResults(CharSequence charSequence, FilterResults filterResults) {
            mTags.clear();
            mTags.addAll((List<String>) filterResults.values);
            onSetTag();
        }
    ...
    public void setTags(List<String> tags) {
        this.mTags = tags;
        if (tags != null) {
            this.originalList = new ArrayList<>(tags);
        }
        onSetTag();
    }
    ...
    public void addTag(String text, int position) {
        if (mTags != null) {
            ...
            if (mTags.size() > minTag) {
                mTags.remove(0);
            }
        }
        if (originalList != null) {
            if (!originalList.contains(text)) {
                originalList.add(text);
                onAddTag(text, position);
                if (mChildViews != null) {
                    if (mChildViews.size() > minTag) {
                        mChildViews.remove(0);
                    }
                }
            }
            if (originalList.size() > minTag) {
    ...
    public void removeTag(int position) {
        removeTagsResultString(position);
    ...
    public List<String> getTagsResult() {
        return originalList;
    }

    public void removeTagsResultString(int position) {
        if (!ValidUtil.isEmpty(originalList)) {
    ...
    /**
     * Set TagView min tag, default 100
     *
     * @param minTag
     */
    public void setMinTag(int minTag) {
        this.minTag = minTag;
    }
