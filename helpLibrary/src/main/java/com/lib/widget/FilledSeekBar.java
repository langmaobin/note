...
public class FilledSeekBar extends View {
    ...
    public interface OnSeekBarChangeListener {
         ...
         */
        void onProgressChanged(FilledSeekBar seekBar, float progress, boolean fromUser);
         ...
         */
        void onStartTrackingTouch(FilledSeekBar seekBar);
         ...
         * @param seekBar The SeekBar in which the touch gesture began
         */
        void onStopTrackingTouch(FilledSeekBar seekBar);

    }

    OnSeekBarChangeListener onSeekBarChangeListener;

    public FilledSeekBar(Context context) {
        super(context);
        init();
    }

    public FilledSeekBar(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public FilledSeekBar(Context context, AttributeSet attrs, int defStyleAttr) {
...
//        progressMargin = thumBitMap.getWidth() / 2 + dip2px(getContext(), 10);
    ...
    int getProgressHeight() {
        if (progressHeight > 0) return progressHeight;
        return dip2px(getContext(), 15);
    }

    int getProgressMargin() {
        if (progressMargin > 0) return progressMargin;
        return dip2px(getContext(), 0);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mLastMotionX <= getProgressMargin() || mLastMotionX <= mOffestX)
...
//    }
    ...
    void drawStartProgressText(Canvas canvas) {
        ...
        if (startProgressText == null) return;
    ...
    void drawMaxProgressText(Canvas canvas) {
        ...
        if (maxProgressText == null) return;
...
//                break;
                case MotionEvent.ACTION_MOVE:
                    ...
                    float progressW = mLastMotionX - getProgressMargin();
                    int bgW = getWidth() - getProgressMargin() * 2;
                    progress = (int) (progressW / (float) bgW * max);
                    if (onSeekBarChangeListener != null)
